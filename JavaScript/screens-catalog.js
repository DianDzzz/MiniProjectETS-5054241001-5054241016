...
function renderHeader(state) {
  ...
  const logo = h('button', {
    ...
    h('div', { style: { width: '18px', height: '18px', background: 'var(--text)', borderRadius: '2px', position: 'relative' } },
      h('div', { style: { position: 'absolute', inset: '4px', background: 'var(--bg)', borderRadius: '1px' } })
    ),
    'NIMBUS'
  );
...
function renderCatalog(state) {
  ...
  const filtersPanel = () => h('div', { style: { display: 'flex', flexDirection: 'column', gap: '24px', fontSize: '13px' } },
    ...
    filterGroup('Harga', [
      ...
      h('input', { type: 'range', min: '0', max: '30000000', step: '100000', value: String(f.priceMax),
    ...
    filterGroup('Brand', BRANDS.map(b =>
      h('label', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 0', cursor: 'pointer' } },
        h('span', { class: `check ${f.brands.includes(b) ? 'on' : ''}`,
    ...
    filterGroup('Stok', [
      h('label', { style: { display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' } },
        h('span', { class: `check ${f.inStock ? 'on' : ''}`,
  ...
  return h('div', {},
    ...
    h('section', {
      ...
      isMobile
        ? [
            h('button', { class: 'btn btn-secondary btn-sm', style: { flex: '1' },
              onclick: () => { state.showFilters = true; state.rerender(); } }, icon('filter', 14), ' Filter'),
            h('button', { class: 'btn btn-secondary btn-sm', style: { flex: '1' },
              onclick: () => { state.showSort = true; state.rerender(); } }, icon('sort', 14), ' Urutkan'),
          ]
        : [
